...
public final class ConcurrentObjectPool<R> implements ObjectPool<R> {
    ...
    public ConcurrentObjectPool(Comparator<R> resourcesComparator) {
        this.availableResources = new PriorityBlockingQueue<>(42, resourcesComparator);
    }
    
    public ConcurrentObjectPool() {
        this.availableResources = new ConcurrentLinkedQueue<>();
    }
    
    //TODO pivanenko refactor to different ordering strategies
//    private ConcurrentLinkedQueue<R> availableResources = new ConcurrentLinkedQueue<>();
    private final Queue<R> availableResources;
